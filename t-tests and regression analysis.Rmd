...
# Making a plot for the regression model WHY DOES THE LINE GET SO WEIRD??
hyp %>%
  ggplot(aes(CLUSTER, AVG_CSA_T1)) + 
  geom_point(size = 3, fill = "lightblue", shape = 21) +
  geom_smooth(method = "lm", se = FALSE) + 
  geom_abline(slope = coef(m1)[2], intercept = coef(m1)[1]) + # The line should go through the mean in LOW and HIGH, why does it not do that?
  labs(x = "Groups",
       y = "Average CSA (cm2)",
