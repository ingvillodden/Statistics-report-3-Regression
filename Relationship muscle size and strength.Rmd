...
#### Null hypothesis
> At T1, there is no correlation between muscle mass measured as average cross-sectional area and muscle strength measured as 3 RM squat

#### Alternative hypothesis
> At T1, there is a positive correlation between muscle mass measured as average cross-sectional area and muscle strength measured as 3 RM squat

## Methods
Pearson's product-moment correlation was used to estimate the correlation between average cross-sectional area and 3 RM squat at T1. r < 0.1 = trivial, 0.1-0.3 = small, 0.3-0.5 = moderate, 0.5-0.7 = large, 0.7-0.9 = very large, 0.9 = almost perfect and 1.0 = perfect correlation [@Hopkins2009]. A regression model was also used to fit the data, with the average cross-sectional area as the predictor, and 3 RM squat as the dependent variable. Tests to check for constant variance, normal residuals and problematic observations were performed. No problematic observations were found, so we can safely use all the observations in the analysis
...
m1 <- lm(Squat_3RM_kg ~ AVG_CSA_T1, data = hyp) # Storing the regression model as m1
...
There was a `r round(corr, 2)` correlation between average cross-sectional area and 3 RM squat at T1 (95% CI: [`r round(ci1, 2)`, `r round(ci2, 2)`], *t*(`r round(df, 1)`) = `r round(t,3)`, *p* = `r round(p, 2)`).
...
As the confidence interval contain 0 (95% CI: [`r round(ci1, 2)`, `r round(ci2, 2)`]), we can not reject the null hypothesis that states that "at T1, there is no correlation between muscle mass measured as average cross-sectional area and muscle strength measured as 3 RM squat." This is supported by the *p*-value (`r round(p, 2)`).

The findings from Haun et al. [-@Haun2019] therefore suggests that there is no correlation between average cross-sectional area and 3 RM squat in trained men before starting a training intervention.
